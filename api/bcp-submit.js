...
function buildCustomerHtml({ company, name, totalScore, level, catPcts, report, lossScenarios, revenueInfo }) {
  ...
  const IMPROVE_SERVICES = [
    ...
    // cat5（サプライチェーン）は当社支援対象外のため非表示
    { cat: 6, name: '災害時を想定した訓練・シミュレーション', icon: '🏃',
      desc: '安否確認訓練・災害対応シミュレーションなど、学んだことを実際の行動に落とし込む実践的プログラムを提供します。' },
  ];

  const ADVANCED_SERVICES = [
    { name: '高度な災害対応シミュレーション演習', icon: '🏆', desc: '大規模複合災害や長期停電など、より難易度の高いシナリオでの演習を実施。現状のBCPの穴を発見し、さらなる実効性強化を図ります。' },
    { name: 'BCP定期見直し・ブラッシュアップ支援', icon: '🔄', desc: '法改正・組織変更・拠点移転など環境変化に合わせて、専門家とともにBCPを継続的にアップデートします。' },
    // サプライチェーン強靭化は当社支援対象外のため非表示
